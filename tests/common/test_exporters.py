...
from onnxtr.io.exporters import (
    AsciiDocExporter,
...
def test_export_mixins_carry_full_api():
    # The element export surface comes from the mixins in doctr.io.exporters, with the API unchange
...
def test_xml_export_edge_cases():
    from onnxtr.io.exporters import _covering_region_indices

    assert _covering_region_indices([], [((0.0, 0.0), (1.0, 1.0))]) == []
    # Rotated geometries are rejected with an explicit error, for blocks and tables  alike
    poly = np.asarray([[0.1, 0.1], [0.4, 0.1], [0.4, 0.2], [0.1, 0.2]], dtype=np.float32)
    rot_page = elements.Page(
        np.zeros((10, 10, 3), dtype=np.uint8),
        [
