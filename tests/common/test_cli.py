...
import onnxtr.cli.main as cli
from onnxtr import __version__
from onnxtr.io import DocumentFile
from onnxtr.io.exporters import DocumentExportsMixin
...
def test_parse_args_defaults():
    ...
    assert args.file_title == "OnnxTR - XML export (hOCR)"
...
NOT_EXPOSED = {
    "self",
    # engine configuration is derived from --device / --load_in_8_bit rather than exposed directly
    "det_engine_cfg",
    "reco_engine_cfg",
    "clf_engine_cfg",
    "layout_engine_cfg",
...
def test_build_predictor_forwards_every_option(recorded_predictor):
    ...
    assert recorded_predictor == {
        ...
        # OnnxTR-only: the engine configuration derived from --device / --load_in_8_bit
        "load_in_8_bit": False,
        "det_engine_cfg": recorded_predictor["det_engine_cfg"],
        "reco_engine_cfg": recorded_predictor["reco_engine_cfg"],
        "clf_engine_cfg": recorded_predictor["clf_engine_cfg"],
        "layout_engine_cfg": recorded_predictor["layout_engine_cfg"],
        "table_engine_cfg": recorded_predictor["table_engine_cfg"],
    }
    # a single engine config is shared by every module
    assert (
        len({
            id(recorded_predictor[k])
            for k in ("det_engine_cfg", "reco_engine_cfg", "clf_engine_cfg", "layout_engine_cfg", "table_engine_cfg")
        })
        == 1
    )
...
# OnnxTR-specific: `--device` maps onto onnxruntime execution providers rather than a torch device


def test_resolve_engine_cfg_cpu():
    args = cli._parse_args([*LIGHT_ARCHS, "--input_path", "x.png", "--device", "cpu"])
    cfg = cli._resolve_engine_cfg(args)
    assert cfg.providers == ["CPUExecutionProvider"]


@pytest.mark.parametrize("value", ["auto", "AUTO", " auto "])
def test_resolve_engine_cfg_auto(value):
    args = cli._parse_args([*LIGHT_ARCHS, "--input_path", "x.png", "--device", value])
    cfg = cli._resolve_engine_cfg(args)
    available = ort.get_available_providers()
    assert cfg.providers == [p for p in ("CUDAExecutionProvider", "CPUExecutionProvider") if p in available]
    assert "CPUExecutionProvider" in cfg.providers


def test_resolve_engine_cfg_cuda_when_unavailable():
    args = cli._parse_args([*LIGHT_ARCHS, "--input_path", "x.png", "--device", "cuda"])
    if "CUDAExecutionProvider" in ort.get_available_providers():
        assert cli._resolve_engine_cfg(args).providers[0] == "CUDAExecutionProvider"
    else:
        with pytest.raises(SystemExit):
            cli._resolve_engine_cfg(args)


def test_resolve_engine_cfg_unknown_provider():
    args = cli._parse_args([*LIGHT_ARCHS, "--input_path", "x.png", "--device", "NotAProvider"])
    with pytest.raises(SystemExit):
        cli._resolve_engine_cfg(args)


def test_resolve_engine_cfg_explicit_provider_name():
    args = cli._parse_args([*LIGHT_ARCHS, "--input_path", "x.png", "--device", "cpuexecutionprovider"])
    assert cli._resolve_engine_cfg(args).providers[0] == "CPUExecutionProvider"


def test_build_predictor_applies_thresholds_and_engine_cfg(recorded_predictor):
    argv = [*LIGHT_ARCHS, "--input_path", "x.png", "--bin_thresh", "0.5", "--box_thresh", "0.25", "--device", "cpu"]
    model = cli._build_predictor(cli._parse_args(argv))
    assert model.det_predictor.model.postprocessor.bin_thresh == 0.5
    assert model.det_predictor.model.postprocessor.box_thresh == 0.25
    kwargs = recorded_predictor
    for key in ("det_engine_cfg", "reco_engine_cfg", "clf_engine_cfg", "layout_engine_cfg", "table_engine_cfg"):
        assert kwargs[key].providers == ["CPUExecutionProvider"]
    assert kwargs["load_in_8_bit"] is False


def test_set_thresholds_without_postprocessor_onnx(monkeypatch, caplog):
    class _Dummy:
        pass

    dummy = _Dummy()
    dummy.det_predictor = _Dummy()
    dummy.det_predictor.model = _Dummy()
    args = cli._parse_args([*LIGHT_ARCHS, "--input_path", "x.png", "--bin_thresh", "0.5"])
    with caplog.at_level(logging.WARNING):
        cli._set_thresholds(dummy, args)
    assert "postprocessor" in caplog.text
